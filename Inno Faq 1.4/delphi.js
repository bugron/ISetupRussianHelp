...
_consts = ["{", "{code:", "{drive:", "{param:", "{ini:", "{reg:", "{ini:", "{cm:", "{%", "{\\", "{app", "{win", "{sys", "{syswow64", "{src", "{sd", "{pf", 
"{pf32", "{pf64", "{cf", "{cf32", "{cf64", "{tmp", "{fonts",
...
_sections = ["[PostCompile", "[PreCompile", "[_ISToolDownload", "[_ISToolPreCompile", "[_ISToolPostCompile", "[ISFormDesigner", "[Setup", "[Types", 
 "[Icons", "[INI", "[InstallDelete", "[Languages", "[Messages", "[CustomMessages", "[LangOptions", "[Run", "[UninstallRun", "[Registry",
 "[UninstallDelete", "[Code", "[Tasks", "[Dirs", "[Files", "[Components"];
...
function setup_delphi() 
{
    ...
    {
		addSimpleEOLComment("///", makeStyle("#008001")); //Comments
		addSimpleEOLComment(";;", makeStyle("#008001")); //Comments
		addSimpleOpencloseComment("{{", "}", makeStyle("#008001")); //Comments
		addSimpleOpencloseComment("(*", "*)", makeStyle("#008001")); //Comments
		addKeywords(_funcs, makeStyle("black", "bold")); //Inno functions
        addKeywords(_flags, makeStyle("#a04000")); //Section flags
        addKeywords(_Parameter, makeStyle("blue")); //Parameters
        addKeywords(_delphi, makeStyle("blue")); //Delphi keywords
        addKeywords(_KeywordOther, makeStyle("blue")); //Inno keywords
		
		var length = _defPrep.length;
		for (var i = 0; i < length; i++)
            addSimpleEOLComment(_defPrep[i], makeStyle("red"));

		length = _consts.length;
        for (var i = 0; i < length; i++)
		...
		addSimpleOpencloseComment("{$", "}", makeStyle("#a04000"));
		addSimpleOpencloseComment("'{#", "}'", makeStyle("red"));
